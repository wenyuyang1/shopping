<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="true">
        ...
        <div class="info-address">
          <!-- 可以渲染出来但是报错且可运行 -->
          {{ defaultAddress.region.province }}
          {{ defaultAddress.region.city }} {{ defaultAddress.region.region }}
    ...
    <div class="footer-fixed">
      <div class="left">
        实付款：<span>￥{{ order.orderPayPrice }}</span>
      </div>
      <div class="tipsbtn" @click="submitGoods">提交订单</div>
    </div>
  </div>
</template>
...
import { getOrderForm, submitOrder } from "@/api/pay";
export default {
  name: "PayIndex",
  data() {
    return {
  ...
  async created() {
    ...
    this.defaultAddress = list[0];
    // 当购买方式为购物车获取时
    let res;
    if (this.goodsMode === "cart") {
      res = await getOrderForm(this.goodsMode, {
        cartIds: this.goodsCartIds,
      });

      // 当购买方式为直接购买时
    } else if (this.goodsMode === "buyNow") {
      res = await getOrderForm(this.goodsMode, {
  ...
  methods: {
    async submitGoods() {
      if (this.goodsMode === "cart") {
        await submitOrder(this.goodsMode, this.remark, {
          cartIds: this.goodsCartIds,
        });
      } else {
        await submitOrder(this.goodsMode, this.remark, {
