  ...
  <div class="prodetail">
    ...
    <div class="comment">
      <div class="comment-title">
        <div class="left">商品评价 ({{ comments_total }}条)</div>
    ...
    <div class="footer">
      ...
      <div class="icon-cart" @click="$router.push('/shoppingCar')">
        <van-icon
      ...
      <div class="btn-add" @click="(showSheet = true), (mode = 'shoppingCart')">
        加入购物车
      </div>
      <div class="btn-buy" @click="(showSheet = true), (mode = 'buyNow')">
        立刻购买
      </div>
    </div>
    ...
    <van-action-sheet
      v-model="showSheet"
      :title="mode === 'shoppingCart' ? '加入购物车 ' : '立即购买'"
    >
      <div class="product">
        <div class="product-title">
          ...
          <div class="right">
            ...
            <div class="count">
              <span>库存</span>
              <span>{{ prodetail.goods_sales }}</span>
            </div>
          </div>
        </div>
        <div class="num-box">
        ...
        <div class="showbtn" v-if="prodetail.stock_total > 0">
          <div class="btn" @click="addCart" v-if="mode === 'shoppingCart'">
            加入购物车
          </div>
...
import hasToken from "@/mixins/hasToken.js";
export default {
  name: "good-prodetail",
  data() {
    return {
  ...
  async created() {
    ...
    let {
      data: { detail },
    } = await getProdetail(this.$route.params.id);
    this.images = detail.goods_images;
    this.prodetail = detail;
    // 获取评论
    let {
      data: { list, total },
    } = await getComment(this.$route.params.id, 3);
    this.comments = list;
    ...
    async addCart() {
      ...
      if (!this.hasToken()) {
        let {
          ...
          this.prodetail.goods_id,
          this.count,
          this.prodetail.skuList[0].goods_sku_id
        );
        this.$toast("添加购物车成功");
        this.shoppingCartNum = cartTotal;
      }
    },
    // 立即购买逻辑脚本
    async buyNow() {
      if (!this.hasToken()) {
        this.$router.push({
          ...
          query: {
            mode: "buyNow",
            goodsId: this.prodetail.goods_id,
            goodsNum: this.count,
